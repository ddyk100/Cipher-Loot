...
import { decryptValue, batchDecryptValues, publicDecrypt, decryptMultipleHandles } from '../core/index.js';
...
export function useDecrypt() {
  ...
  const batchDecrypt = useCallback(async (handles: string[], contractAddress: string, signer: any) => {
    setIsDecrypting(true);
    setError('');
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Batch decryption failed');
      throw err;
    } finally {
      setIsDecrypting(false);
    }
